...
export class LibraryController {
  ...
  @Get(':videoId') async detail(@Param('videoId') id: string, @Res({ passthrough: true }) response: FastifyReply) { const result = await this.run(() => this.library.detail(id)); response.header('ETag', `"${result.data.version}"`); return result; }
