...
export class WorkerAgentController {
  ...
  @Post('sessions/:id/heartbeat') @HttpCode(200) heartbeat(@Headers('authorization') auth: string | undefined, @Headers('idempotency-key') key: string | undefined, @Param('id') id: string, @Body() body: HeartbeatDto) { requiredKey(key); return this.run(() => this.workers.heartbeat(bearer(auth), id, body)); }
