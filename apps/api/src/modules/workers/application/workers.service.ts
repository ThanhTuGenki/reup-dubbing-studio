...
function validateImage(input: CreateImageInput) { if (!/^sha256:[a-f0-9]{64}$/u.test(input.imageDigest) || !/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/u.test(input.semanticVersion) || input.contractVersion < 1) throw new WorkerError('WORKER_VALIDATION_FAILED', 'Worker image metadata is invalid'); }
...
function validateEnrollment(input: EnrollmentInput) { uuid(input.sessionNonce); if (!/^sha256:[a-f0-9]{64}$/u.test(input.imageDigest) || input.contractVersion < 1 || !input.agentVersion.trim()) throw new WorkerError('WORKER_VALIDATION_FAILED', 'Enrollment identity is invalid'); }
function validateHeartbeat(input: HeartbeatInput) { let sequence: bigint; try { sequence = BigInt(input.sequence); } catch { throw new WorkerError('WORKER_VALIDATION_FAILED', 'Heartbeat sequence is invalid'); } if (sequence < 0n || input.currentTaskCount < 0 || Number.isNaN(new Date(input.sentAt).valueOf())) throw new WorkerError('WORKER_VALIDATION_FAILED', 'Heartbeat payload is invalid'); input.activeLeaseIds.forEach(uuid); }
