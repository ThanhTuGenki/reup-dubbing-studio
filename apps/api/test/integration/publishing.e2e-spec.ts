...
describeWithDatabase('Manual Publishing API with PostgreSQL', () => {
  ...
  it('retains field revisions, enforces lock/policy/checklist and completes required siblings independently', async () => {
    ...
    for (const task of packageRow.tasks) {
      ...
      const incomplete = await app.inject({ method: 'POST', url: `/v1/publication-tasks/${task.id}/proofs`, headers: { 'if-match': `"${version}"`, 'idempotency-key': uuidV7() }, payload: { publicUrl: `https://example.test/posts/${task.id}` } }); expect(incomplete.statusCode).toBe(422); expect(incomplete.json()).toMatchObject({ code: 'PUBLICATION_CHECKLIST_INCOMPLETE' });
      ...
      const credentialUrl = await app.inject({ method: 'POST', url: `/v1/publication-tasks/${task.id}/proofs`, headers: { 'if-match': `"${version}"`, 'idempotency-key': uuidV7() }, payload: { publicUrl: `https://operator:secret@example.test/posts/${task.id}` } }); expect(credentialUrl.statusCode).toBe(422); expect(credentialUrl.json()).toMatchObject({ code: 'PUBLICATION_PROOF_REQUIRED' });
      const unsafePostId = await app.inject({ method: 'POST', url: `/v1/publication-tasks/${task.id}/proofs`, headers: { 'if-match': `"${version}"`, 'idempotency-key': uuidV7() }, payload: { platformPostId: 'post?token=secret' } }); expect(unsafePostId.statusCode).toBe(422); expect(unsafePostId.json()).toMatchObject({ code: 'PUBLICATION_PROOF_REQUIRED' });
      ...
      const oldProofId = retryProof.json().data.proofs[1].id; const oldProof = await app.inject({ method: 'POST', url: `/v1/publication-tasks/${task.id}/proofs/${oldProofId}/verify`, headers: { 'if-match': `"${version}"`, 'idempotency-key': uuidV7() }, payload: { status: 'VERIFIED' } }); expect(oldProof.statusCode).toBe(422); expect(oldProof.json()).toMatchObject({ code: 'PUBLICATION_PROOF_REQUIRED' });
      const proofId = retryProof.json().data.proofs[0].id; const verified = await app.inject({ method: 'POST', url: `/v1/publication-tasks/${task.id}/proofs/${proofId}/verify`, headers: { 'if-match': `"${version}"`, 'idempotency-key': uuidV7() }, payload: { status: 'VERIFIED' } }); expect(verified.statusCode).toBe(200); expect(verified.json().data.status).toBe('VERIFIED');
