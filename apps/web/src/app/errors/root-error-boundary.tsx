...
export class RootErrorBoundary extends Component<PropsWithChildren, { hasError: boolean }> {
  ...
  public override render(): ReactNode {
    if (this.state.hasError) return <main className="fatal-fallback"><div className="fallback-card" role="alert"><p className="eyebrow">Reup Dubbing Studio</p><h1>Ứng dụng chưa thể hiển thị</h1><p>Hãy tải lại trang. Nếu lỗi vẫn tiếp diễn, liên hệ người vận hành hệ thống.</p><button className="button button-primary" type="button" onClick={() => window.location.reload()}>Tải lại trang</button></div></main>;
