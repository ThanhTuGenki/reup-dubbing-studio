...
export function StudioPage() {
  ...
  const save = useMutation({ mutationFn: () => selected && studio ? saveStudioSegment(videoId, selected.id, studio.video.version, { translatedText: selectedText, editReason: 'Web Studio edit' }) : Promise.reject(new Error('Chưa chọn segment')), onSuccess: () => { setDrafts({}); void queryClient.invalidateQueries({ queryKey: studioKeys.detail(videoId) }); } });
  ...
  return <div className="page studio-page">
    ...
    <section className="queue-list-card studio-editor"><div className="studio-section-heading"><div><p className="eyebrow">Transcript editor</p><h2>{studio.segments.length} đoạn thoại</h2></div><Badge variant={dirty ? 'secondary' : 'outline'}>{dirty ? 'Chưa lưu thay đổi' : 'Đã đồng bộ'}</Badge></div><div className="studio-editor-layout"><nav className="studio-segment-list" aria-label="Danh sách segment">{studio.segments.map((item) => <button key={item.id} type="button" className={item.id === selected?.id ? 'is-active' : ''} onClick={() => setSelectedId(item.id)}><span>{String(item.ordinal).padStart(2, '0')}</span><span>{item.revision?.translatedText || 'Chưa có bản dịch'}</span><Badge variant="outline">{item.revision?.status ?? 'MISSING'}</Badge></button>)}</nav>{selected && <div className="studio-segment-form"><div className="studio-segment-meta"><Badge variant="outline">{formatDuration(selected.sourceStartMs)}–{formatDuration(selected.sourceEndMs)}</Badge><span>Revision {selected.revision?.revision ?? '—'}</span><span>{selected.revision?.voice.name ?? 'Chưa có voice'}</span></div><label><span>Bản dịch</span><Textarea aria-label="Bản dịch segment" value={selectedText} onChange={(event) => setDrafts((current) => ({ ...current, [selected.id]: event.target.value }))} rows={5} /></label><div className="studio-form-actions"><Button onClick={() => save.mutate()} disabled={!dirty || save.isPending}><SaveIcon />{save.isPending ? 'Đang lưu…' : 'Lưu revision'}</Button><Button variant="outline" onClick={() => preview.mutate()} disabled={preview.isPending || !selected.revision?.preview}><Volume2Icon />{preview.isPending ? 'Đang tạo…' : 'Nghe preview'}</Button><Button variant="outline" onClick={() => regenerate.mutate()} disabled={regenerate.isPending || !studio.capabilities.canRegenerate}><RefreshCwIcon />Re-generate</Button></div></div>}</div></section>
    <section className="queue-list-card studio-review"><div className="studio-section-heading"><div><p className="eyebrow">Review gate</p><h2>Duyệt bản dựng hiện tại</h2></div><Clock3Icon aria-hidden="true" /></div><div className="studio-review-form"><label><span>Quyết định</span><NativeSelect value={reviewDecision} onChange={(event) => setReviewDecision(event.target.value as typeof reviewDecision)}><NativeSelectOption value="APPROVED">Approve</NativeSelectOption><NativeSelectOption value="CHANGES_REQUESTED">Yêu cầu chỉnh sửa</NativeSelectOption><NativeSelectOption value="REJECTED">Reject</NativeSelectOption></NativeSelect></label><label><span>Ghi chú</span><Input value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} placeholder="Tuỳ chọn" /></label><Button onClick={() => review.mutate()} disabled={review.isPending}><CheckCircle2Icon />{review.isPending ? 'Đang lưu…' : 'Lưu review'}</Button></div></section>
  </div>;
}
