...
ARG BASE_IMAGE
ENV VIRTUAL_ENV=/opt/venv UV_PROJECT_ENVIRONMENT=/opt/venv PATH="/opt/venv/bin:/usr/local/bin:$PATH"
...
USER worker
WORKDIR /workspace
HEALTHCHECK --interval=30s --timeout=3s CMD ["reup-batch-worker", "--health"]
ENTRYPOINT ["reup-batch-worker"]
